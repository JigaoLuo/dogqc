...
__global__ void krnl_lineitem1(
    int* iatt5_llinenum, int* iatt6_lquantit, agg_ht<apayl2>* aht2, double* agg1) {
    int att5_llinenum;
    int att6_lquantit;
    ...
    while(!(flushPipeline)) {
        ...
        if(active) {
            atomicAdd(&(agg1[bucket]), ((double)att6_lquantit));
        }
        loopVar += step;
    }
...
__global__ void krnl_aggregation2(
    agg_ht<apayl2>* aht2, double* agg1, int* nout_result, int* oatt5_llinenum, double* oatt1_sumlquan) {
    int att5_llinenum;
    double att1_sumlquan;
    ...
    while(!(flushPipeline)) {
        ...
        if(active) {
            att1_sumlquan = agg1[tid_aggregation2];
        }
        ...
        if(active) {
            ...
            oatt1_sumlquan[wp] = att1_sumlquan;
        }
        loopVar += step;
    }
...
int main() {
    ...
    std::vector < double > oatt1_sumlquan(6001215);
    ...
    double* d_oatt1_sumlquan;
    cudaMalloc((void**) &d_oatt1_sumlquan, 6001215* sizeof(double) );
    cudaDeviceSynchronize();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    std::clock_t start_krnl_lineitem11 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
        krnl_lineitem1<<<gridsize, blocksize>>>(d_iatt5_llinenum, d_iatt6_lquantit, d_aht2, d_agg1);
    }
    cudaDeviceSynchronize();
    ...
    std::clock_t start_krnl_aggregation22 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
        krnl_aggregation2<<<gridsize, blocksize>>>(d_aht2, d_agg1, d_nout_result, d_oatt5_llinenum, d_oatt1_sumlquan);
    }
    cudaDeviceSynchronize();
    ...
    cudaMemcpy( oatt1_sumlquan.data(), d_oatt1_sumlquan, 6001215 * sizeof(double), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    for ( int pv = 0; ((pv < 10) && (pv < nout_result)); pv += 1) {
        ...
        printf("  ");
        printf("sum_l_quantity: ");
        printf("%15.2f", oatt1_sumlquan[pv]);
        printf("  ");
        printf("\n");
    }
    if((nout_result > 10)) {
