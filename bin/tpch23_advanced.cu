...
__global__ void krnl_reduce1(  ///
    int* oatt5_llinenum, int* oatt1_countlli, agg_ht<apayl2>* aht2, int* agg1, int* nout_result, int* d_reduce_hash_table_size) {  ///
    int att5_llinenum;  ///
    int att1_countlli;  ///
...
__global__ void krnl_reduce2(
        agg_ht<apayl2>* aht2, int* agg1, int* n_final_out_result, int* oatt5_llinenum, int* oatt1_countlli, int* d_reduce_hash_table_size) {  ///
    int att5_llinenum;  ///
    int att1_countlli;  ///
    unsigned warplane = (threadIdx.x % 32);
    unsigned prefixlanes = (0xffffffff >> (32 - warplane));
...
int main() {
    ...
    int reduce_hash_table_size = next_power_2(nout_result);  ///
    ...
    std::clock_t start_krnl_reduce1 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
        krnl_reduce1<<<gridsize, blocksize>>>(d_oatt5_llinenum, d_oatt1_countlli, d_aht2, d_agg1, d_nout_result, d_reduce_hash_table_size);
    }
    cudaDeviceSynchronize();
    ...
    std::clock_t start_krnl_reduce2 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
        krnl_reduce2<<<gridsize, blocksize>>>(d_aht2, d_agg1, d_final_nout_result, d_final_oatt5_llinenum, d_final_oatt1_countlli, d_reduce_hash_table_size);
    }
    cudaDeviceSynchronize();
    ...
    cudaMemcpy( oatt5_llinenum.data(), d_final_oatt5_llinenum, 6001215 * sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy( oatt1_countlli.data(), d_final_oatt1_countlli, 6001215 * sizeof(int), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
