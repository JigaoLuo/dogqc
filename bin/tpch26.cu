...
__global__ void krnl_lineitem1(
    int* iatt5_llinenum, int* iatt6_lquantit, agg_ht<apayl2>* aht2, float* agg1) {
    int att5_llinenum;
    int att6_lquantit;
    ...
    while(!(flushPipeline)) {
        ...
        if(active) {
            atomicAdd(&(agg1[bucket]), ((float)att6_lquantit));
        }
        loopVar += step;
    }
...
__global__ void krnl_aggregation2(
    agg_ht<apayl2>* aht2, float* agg1, int* nout_result, int* oatt5_llinenum, float* oatt1_sumlquan) {
    int att5_llinenum;
    float att1_sumlquan;
...
int main() {
    ...
    std::vector < float > oatt1_sumlquan(6001215);
    ...
    float* d_oatt1_sumlquan;
    cudaMalloc((void**) &d_oatt1_sumlquan, 6001215* sizeof(float) );
    cudaDeviceSynchronize();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    float* d_agg1;
    cudaMalloc((void**) &d_agg1, 12002430* sizeof(float) );
    {
        int gridsize=920;
        int blocksize=128;
    ...
    cudaMemcpy( oatt1_sumlquan.data(), d_oatt1_sumlquan, 6001215 * sizeof(float), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
