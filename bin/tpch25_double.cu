...
__global__ void krnl_lineitem1(
    int* iatt4_lsuppkey, int* iatt5_llinenum, agg_ht<apayl2>* aht2, double* agg1) {
    int att4_lsuppkey;
    int att5_llinenum;
    ...
    while(!(flushPipeline)) {
        ...
        if(active) {
            atomicAdd(&(agg1[bucket]), ((double)att4_lsuppkey));
        }
        loopVar += step;
    }
...
__global__ void krnl_aggregation2(
    agg_ht<apayl2>* aht2, double* agg1, int* nout_result, int* oatt5_llinenum, double* oatt1_sumlsupp) {
    int att5_llinenum;
    double att1_sumlsupp;
    ...
    while(!(flushPipeline)) {
        ...
        if(active) {
            att1_sumlsupp = agg1[tid_aggregation2];
        }
        ...
        if(active) {
            ...
            oatt1_sumlsupp[wp] = att1_sumlsupp;
        }
        loopVar += step;
    }
...
int main() {
    ...
    std::vector < double > oatt1_sumlsupp(6001215);
    ...
    double* d_oatt1_sumlsupp;
    cudaMalloc((void**) &d_oatt1_sumlsupp, 6001215* sizeof(double) );
    cudaDeviceSynchronize();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    double* d_agg1;
    cudaMalloc((void**) &d_agg1, 12002430* sizeof(double) );
    {
        int gridsize=920;
        int blocksize=128;
        initArray<<<gridsize, blocksize>>>(d_agg1, 0.0d, 12002430);
    }
    {
        int gridsize=920;
        int blocksize=128;
    ...
    std::clock_t start_krnl_lineitem11 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
        krnl_lineitem1<<<gridsize, blocksize>>>(d_iatt4_lsuppkey, d_iatt5_llinenum, d_aht2, d_agg1);
    }
    cudaDeviceSynchronize();
    ...
    std::clock_t start_krnl_aggregation22 = std::clock();
    {
        int gridsize=920;
        int blocksize=128;
        krnl_aggregation2<<<gridsize, blocksize>>>(d_aht2, d_agg1, d_nout_result, d_oatt5_llinenum, d_oatt1_sumlsupp);
    }
    cudaDeviceSynchronize();
    ...
    cudaMemcpy( oatt1_sumlsupp.data(), d_oatt1_sumlsupp, 6001215 * sizeof(double), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    for ( int pv = 0; ((pv < 10) && (pv < nout_result)); pv += 1) {
        ...
        printf("  ");
        printf("sum_l_suppkey: ");
        printf("%15.2f", oatt1_sumlsupp[pv]);
        printf("  ");
        printf("\n");
    }
    if((nout_result > 10)) {
