...
__global__ void krnl_lineitem1(
    int* iatt4_lsuppkey, int* iatt5_llinenum, agg_ht<apayl2>* aht2, float* agg1) {
    int att4_lsuppkey;
    int att5_llinenum;
    ...
    while(!(flushPipeline)) {
        ...
        if(active) {
            atomicAdd(&(agg1[bucket]), ((float)att4_lsuppkey));
        }
        loopVar += step;
    }
...
__global__ void krnl_aggregation2(
    agg_ht<apayl2>* aht2, float* agg1, int* nout_result, int* oatt5_llinenum, float* oatt1_sumlsupp) {
    int att5_llinenum;
    float att1_sumlsupp;
...
int main() {
    ...
    std::vector < float > oatt1_sumlsupp(6001215);
    ...
    float* d_oatt1_sumlsupp;
    cudaMalloc((void**) &d_oatt1_sumlsupp, 6001215* sizeof(float) );
    cudaDeviceSynchronize();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
    ...
    cudaMemcpy( oatt1_sumlsupp.data(), d_oatt1_sumlsupp, 6001215 * sizeof(float), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
    {
        cudaError err = cudaGetLastError();
        if(err != cudaSuccess) {
